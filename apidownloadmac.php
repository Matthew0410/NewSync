<?php
	...
	foreach ($arr_data as $day => $data)
	{
		...
		foreach ($data as $k => $v)
		{
			...
			/*========= Rate 3 ========= */
			$phpexcel->getActiveSheet()->SetCellValue("I{$row}", "=IF(D{$row} > 0, G{$row}/D{$row}, 0)");
			$phpexcel->getActiveSheet()->getStyle("I{$row}")->getNumberFormat()->applyFromArray(array(
				'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
			));

			/*========= Rate 4 ========= */
			$phpexcel->getActiveSheet()->SetCellValue("J{$row}", $v->success);
	...
	$phpexcel->getActiveSheet()->SetCellValue("I{$row}", "=IF(D{$last_rownu} > 0, G{$last_rownu}/D{$last_rownu})");
	$phpexcel->getActiveSheet()->getStyle("I{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_style_NumberFormat::FORMAT_PERCENTAGE
	));
	$phpexcel->getActiveSheet()->SetCellValue("F{$row}", "=IF(D{$last_rownu} > 0, E{$last_rownu}/D{$last_rownu}, 0)");
	$phpexcel->getActiveSheet()->getStyle("F{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));
	$phpexcel->getActiveSheet()->SetCellValue("H{$row}", "=IF(E{$last_rownu} > 0, G{$last_rownu}/E{$last_rownu}, 0)");

	$phpexcel->getActiveSheet()->getStyle("H{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));
	$phpexcel->getActiveSheet()->SetCellValue("J{$row}", "=SUM(J{$first_row}:J{$last_row})");
	$phpexcel->getActiveSheet()->SetCellValue("K{$row}", "=IF(G{$last_rownu} > 0, J{$last_rownu}/G{$last_rownu}, 0)");
	$phpexcel->getActiveSheet()->getStyle("K{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	$phpexcel->getActiveSheet()->SetCellValue("L{$row}", "=IF(E{$last_rownu} > 0, J{$last_rownu}/E{$last_rownu}, 0)");
	$phpexcel->getActiveSheet()->getStyle("L{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	$phpexcel->getActiveSheet()->SetCellValue("M{$row}", "=IF(D{$last_rownu} > 0, J{$last_rownu}/D{$last_rownu}, 0)");
	$phpexcel->getActiveSheet()->getStyle("M{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));
	...
	/*========= Average All Column =========*/
	$phpexcel->getActiveSheet()->SetCellValue("B{$row}", 'AVERAGE 2');

	$phpexcel->getActiveSheet()->mergeCells("B{$row}:C{$row}");

	// $phpexcel->getActiveSheet()->SetCellValue("E{$row}", );
	...
	$phpexcel->getActiveSheet()->SetCellValue("G{$row}", 'Rate 7');
	$phpexcel->getActiveSheet()->SetCellValue("H{$row}", '> 0 Mac');
	$phpexcel->getActiveSheet()->SetCellValue("I{$row}", 'Rate 8');
	$phpexcel->getActiveSheet()->SetCellValue("J{$row}", '1 Mac');
	$phpexcel->getActiveSheet()->SetCellValue("K{$row}", 'Rate 9');
	$phpexcel->getActiveSheet()->SetCellValue("L{$row}", '> 1');
	$phpexcel->getActiveSheet()->SetCellValue("M{$row}", 'Rate 10');
	$phpexcel->getActiveSheet()->SetCellValue("N{$row}", 'TB Mac');
	$phpexcel->getActiveSheet()->SetCellValue("O{$row}", 'Rate 11');
	$phpexcel->getActiveSheet()->SetCellValue("P{$row}", 'TVC');
	$phpexcel->getActiveSheet()->SetCellValue("Q{$row}", 'Rate 12');
	$phpexcel->getActiveSheet()->SetCellValue("R{$row}", 'SP');
	$phpexcel->getActiveSheet()->SetCellValue("S{$row}", 'Rate 13');
	...
	// Summary of rate 7
	$phpexcel->getActiveSheet()->SetCellValue("G{$row}", "=F{$last_row2}/C{$last_row2}");
	$phpexcel->getActiveSheet()->getStyle("G{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 8
	$phpexcel->getActiveSheet()->SetCellValue("I{$row}", "=H{$last_row2}/C{$last_row2}");
	$phpexcel->getActiveSheet()->getStyle("I{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 9
	$phpexcel->getActiveSheet()->SetCellValue("K{$row}", "=J{$last_row2}/H{$last_row2}");
	$phpexcel->getActiveSheet()->getStyle("K{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 10
	$phpexcel->getActiveSheet()->SetCellValue("M{$row}", "=L{$last_row2}/H{$last_row2}");
	$phpexcel->getActiveSheet()->getStyle("M{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 12
	$phpexcel->getActiveSheet()->SetCellValue("Q{$row}", "=P{$last_row2}/C{$last_row2}");
	$phpexcel->getActiveSheet()->getStyle("Q{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 13
	$phpexcel->getActiveSheet()->SetCellValue("S{$row}", "=R{$last_row2}/C{$last_row2}");
	$phpexcel->getActiveSheet()->getStyle("S{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));
	...
	$last_row7 = $row - 1;
	$phpexcel->getActiveSheet()->SetCellValue("C{$row}", "=AVERAGE(C{$last_row7}/E{$last_row7})");
